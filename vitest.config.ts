import { defineConfig, mergeConfig } from "vitest/config";
import baseConfig from "./vite.config";

const isStorybook = process.env.VITEST_STORYBOOK === "true";
const storybookConfigDir = process.env.STORYBOOK_CONFIG_DIR?.replace(
  /\\/g,
  "/",
);

const rootDir = process.cwd();
const posixRoot = rootDir.replace(/\\/g, "/");
const storybookInclude = `${posixRoot}/src/**/*.stories.@(js|ts|svelte)`;

const storybookTestConfig = isStorybook
  ? {
      root: rootDir,
      test: {
        name: storybookConfigDir
          ? `storybook:${storybookConfigDir}`
          : undefined,
        include: [storybookInclude],
        setupFiles: ["./.storybook/vitest.setup.ts"],
        environment: "jsdom",
      },
    }
  : {};

export default mergeConfig(baseConfig, defineConfig(storybookTestConfig));
